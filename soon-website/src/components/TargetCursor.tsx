...
export interface TargetCursorProps {
  ...
  /** When false, the custom cursor fades out and the OS cursor is restored,
   *  without unmounting (so it can fade back in). Defaults to true. */
  active?: boolean;
  /** Live pointer position (viewport px). Used to seed the cursor's starting
   *  position (instead of window centre) so it never flashes mid-screen when it
   *  activates under a stationary pointer. */
  pointerRef?: React.RefObject<{ x: number; y: number } | null>;
}
...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    // Body-cursor hiding is handled reactively by the `active` effect below.
    ...
    gsap.set(cursor, {
      xPercent: -50,
      yPercent: -50,
      x: (seed ? seed.x : window.innerWidth / 2) - initialOffset.x,
      y: (seed ? seed.y : window.innerHeight / 2) - initialOffset.y
    });
  ...
  // Reactively hide/restore the OS cursor as `active` toggles (so the component
  // can stay mounted and fade, instead of unmounting to restore the cursor).
  useEffect(() => {
    if (isMobile) return;
    const shouldHide = active && hideDefaultCursor;
    const previous = document.body.style.cursor;
    document.body.style.cursor = shouldHide ? 'none' : '';
    return () => {
      document.body.style.cursor = previous;
    };
  }, [active, hideDefaultCursor, isMobile]);

  // Snap to the real pointer whenever the cursor (re)activates so it never fades
  // in at a stale/centre position under a stationary pointer.
  useEffect(() => {
    if (isMobile || !active || !cursorRef.current || !pointerRef?.current) return;
    const { x: ox, y: oy } = getContainingBlockOffset(containingBlockRef.current);
    gsap.set(cursorRef.current, {
      x: pointerRef.current.x - ox,
      y: pointerRef.current.y - oy
    });
  }, [active, isMobile, pointerRef]);
