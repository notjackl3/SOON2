...
interface ShapeGridProps {
  direction?: 'diagonal' | 'up' | 'right' | 'down' | 'left' | 'none';
  speed?: number;
  ...
  /** Color the grid fades into at the canvas edges. Pass a transparent value
   *  (e.g. `rgba(0,0,0,0)`) to disable the built-in vignette and fade via a
   *  CSS mask instead. */
  fadeColor?: string;
}
...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // Backing store at device resolution, displayed at CSS size, and draw in
      // CSS pixels via the transform — otherwise the bitmap is upscaled (blurry).
    ...
    const drawGrid = () => {
      ...
      if (isHex) {
        ...
        // Snap stroke coords to the device-pixel grid so the 1px lines land on
        // whole pixels (crisp) instead of straddling two (anti-aliased/blurry).
        // For odd device line widths the line must be centered on a half pixel.
        ctx.lineWidth = 1;
        const halfPx = Math.round(dpr) % 2 === 1 ? 0.5 / dpr : 0;
        const snap = (v: number) => Math.round(v * dpr) / dpr + halfPx;

        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            const sx = snap(col * squareSize + offsetX);
            const sy = snap(row * squareSize + offsetY);
    ...
    // Keep the canvas bitmap matched to its displayed box, otherwise the
